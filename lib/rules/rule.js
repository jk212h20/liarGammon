 ...
 */
 ...
 Rule.prototype.realrollDice = function(game, values) {
   console.log('realroll');
   // Create dice object with 2 random values
  var realdice = [Math.floor(Math.random()*6+1),Math.floor(Math.random()*6+1)];
...
Rule.prototype.rollDice = function(game, values) {
  // Create dice object with 2 random values
  console.log('here be values',values);
  var dice = model.Dice.roll();
  console.log('here be dice',dice);
  if (typeof values !== "undefined") {
    dice.values[0] = game.realturnDice[0];
    dice.values[1] = game.realturnDice[1];
  }
  dice.values[0] = game.realturnDice[0];
  dice.values[1] = game.realturnDice[1];
  ...
  //Setting these values doesn't change the moves subarray which still holds the rolled numbers
  return dice;
};
